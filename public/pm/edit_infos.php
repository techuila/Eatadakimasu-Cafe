		...
		if($_POST['password']==$_POST['passverif'])
		{
			...
			if(strlen($_POST['password'])>=6)
			{
				...
				if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
				{
					...
					$dn = mysqli_fetch_array(mysqli_query($connect,'select count(*) as nb from users where username="'.$username.'"'));
...
<?php
	...
	if($form)
	{
		...
		else
		{
			...
			$dnn = mysqli_fetch_array(mysqli_query($connect,'select username,password,email,avatar from users where username="'.$_SESSION['username'].'"'));
